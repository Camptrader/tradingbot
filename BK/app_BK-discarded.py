...
from param_registry import load_best_params, load_all_best_params, register_best_params
    ...
    with st.expander("Data Feed", expanded=False):
        ...
        if df is not None and not df.empty:
            ...
            if 'date' not in df.columns:
                ...
                test_range = st.date_input("Select Backtest Period", value=(data_start, data_end),
    ...
    with st.expander("Optimizer Settings", expanded=False):
        ...
        if optimizer_choice == "Optuna (Bayesian)":
            ...
            Coordinate_Descent_trials = st.number_input("Coordinate Descent Trials", min_value=1, max_value=50, value=4,
                                                        step=1)
        run_optuna = st.button("Run Optimization")
...
# --- Auto-prefill at start/change
prefill_key = f"{strategy_choice}|{selected_symbol}|{selected_tf}"
if 'param_prefilled' not in st.session_state or st.session_state.get("last_prefill_key", None) != prefill_key:
    best_params = load_best_params(strategy_choice, selected_symbol, selected_tf)
    for p in strategy_params + universal_keys:
        k = f"param_{p}"
        if best_params and p in best_params:
            st.session_state[k] = best_params[p]
        elif k not in st.session_state:
            st.session_state[k] = DEFAULTS.get(p)
    st.session_state["param_prefilled"] = True
    st.session_state["last_prefill_key"] = prefill_key


# ---- Editable Parameter Table with Save Button and Results ----

input_col, result_col = st.columns([3, 2])

# --- LOAD BEST PARAMS BUTTON (insert above your parameter grid!) ---
# --- If flagged, load best params and rerun before widget creation ---
if st.session_state.get("trigger_load_params", False):
    best_params = load_best_params(strategy_choice, selected_symbol, selected_tf)
    if best_params:
        for p in strategy_params:
            ...
            if p in best_params:
                st.session_state[k] = best_params[p]
        for p in universal_keys:
            k = f"param_{p}"
            if p in best_params:
                st.session_state[k] = best_params[p]
        st.session_state["trigger_load_params"] = False
        st.rerun()
...
if st.button("Load Best Params"):
    st.session_state["trigger_load_params"] = True
    st.rerun()
    best_params = load_best_params(strategy_choice, selected_symbol, selected_tf)
    if best_params:
        for p in strategy_params:
        ...
        st.warning("No previous best parameters found.")
...
with input_col:
    ...
    for idx, p in enumerate(strategy_params):
        k = f"param_{p}"
        col = strat_cols[idx % 2]
        param_default = DEFAULTS.get(p)
        # Always get the *current* value (user may have edited in session)
        v = st.session_state.get(k, param_default)
        with col:
            if isinstance(param_default, bool):
                param_values[p] = st.checkbox(label=p, value=bool(v), key=k)
            elif isinstance(param_default, int) and not isinstance(param_default, bool):
                # Cast value to int for integer fields!
                val = int(v) if v is not None else 0
                param_values[p] = st.number_input(label=p, value=val, key=k, format="%d")
            elif isinstance(param_default, float):
                val = float(v) if v is not None else 0.0
                param_values[p] = st.number_input(label=p, value=val, key=k, format="%.5f")
            else:
                param_values[p] = st.text_input(label=p, value=str(v) if v is not None else "", key=k)

    # --- Universal/Optimizer Params ---
    universal_param_values = {}
    st.markdown("**Optimizer/Universal Parameters**")
    opt_cols = st.columns(2)
    for idx, p in enumerate(universal_keys):
        k = f"param_{p}"
        col = opt_cols[idx % 2]
        param_default = DEFAULTS.get(p)
        v = st.session_state.get(k, param_default)
        with col:
            if isinstance(param_default, bool):
                universal_param_values[p] = st.checkbox(label=p, value=bool(v), key=k)
            elif isinstance(param_default, int) and not isinstance(param_default, bool):
                val = int(v) if v is not None else 0
                universal_param_values[p] = st.number_input(label=p, value=val, key=k, format="%d")
            elif isinstance(param_default, float):
                val = float(v) if v is not None else 0.0
                universal_param_values[p] = st.number_input(label=p, value=val, key=k, format="%.5f")
            else:
                universal_param_values[p] = st.text_input(label=p, value=str(v) if v is not None else "", key=k)

    # ---- Save Button ----
    if st.button("Save These Parameters as Best"):
        st.session_state["trigger_load_params"] = True
        st.rerun()
        all_params = {**param_values, **universal_param_values}
        metrics_dict = {"manual_add": True, "date": datetime.utcnow().isoformat()}
        register_best_params(strategy_choice, selected_symbol, selected_tf, all_params, metrics_dict)
        # Update session state with just-saved params and rerun to show them!
        for p in strategy_params:
            k = f"param_{p}"
            st.session_state[k] = all_params.get(p, st.session_state[k])
        for p in universal_keys:
            k = f"param_{p}"
            st.session_state[k] = all_params.get(p, st.session_state[k])
        st.success("Parameters saved as best!")
        st.rerun()

with result_col:
    st.subheader("Best Results")
    history = load_all_best_params(strategy_choice, selected_symbol, selected_tf)
    if history:
        best_metrics = history[0]['metrics']
        st.dataframe(pd.DataFrame([best_metrics]))
    else:
        st.info("No results yet for this combo.")

# ---- Parameter History ----
with st.expander("Best Parameter History", expanded=False):
    history = load_all_best_params(strategy_choice, selected_symbol, selected_tf)
    if history:
        st.markdown("#### All Saved Runs:")
        for entry in history:
            metrics_str = ", ".join([f"{k}: {v:.2f}" if isinstance(v, float) else f"{k}: {v}" for k, v in entry['metrics'].items()])
            params_str = ", ".join([f"{k}: {v}" for k, v in entry['params'].items()])
            st.markdown(f"- **Date:** `{entry['date']}`  \n  **Params:** `{params_str}`  \n  **Metrics:** `{metrics_str}`")
    else:
        st.info("No saved parameter runs for this combo.")

# --- After this point, keep your optimizer, backtest, and result display code as you have it ---

# ---- Coordinate Descent Optimizer (and run section) stays unchanged ----
# ... [the rest of your optimizer and run/result code remains as you have it] ...
...
def coordinate_descent_optimizer(
        ...
        n_rounds=Coordinate_Descent_trials,# /////////////////////////////////////////////////////////////////////////////set rounds
        show_progress=True
):
    current_params = {}
...
if run_optuna and df is not None and not df.empty:
    ...
    if not trades.empty:
        metrics_dict = {
            "return": trades['return'].sum() if 'return' in trades else 0,
            "win_pct": (trades['pnl'] > 0).mean() * 100 if 'pnl' in trades else 0,
            "num_trades": len(trades)
            # Add Sharpe, Sortino, etc if available
        }
        register_best_params(
            strategy_choice,
            selected_symbol,
            selected_tf,
            best_params if optimizer_choice == "Optuna (Bayesian)" else best_params,
            metrics_dict
        )
    # --- Display results ---/////////////////////////////////////////////////////////////////////////////////////////
